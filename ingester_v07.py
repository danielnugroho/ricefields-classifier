...
def get_bounds(envi_header):
    ...
    min_x = center_lon - (center_pixel_x - 0.5) * pixel_width
    max_y = center_lat + (center_pixel_y - 0.5) * pixel_height
    
    # Bottom-right corner (max_x, min_y)
    max_x = center_lon + (samples - center_pixel_x - 0.5) * pixel_width
    min_y = center_lat - (lines - center_pixel_y - 0.5) * pixel_height
